      ...
      {axes.map((a, i) => {
        ...
        return (
          <g key={i}>
            ...
            <text x={lbl.x} y={lbl.y} textAnchor="middle" dominantBaseline="middle" fill="#555" fontSize="6" fontFamily="'Geist Mono', monospace">{a.label}</text>
          </g>
        );
      })}
    ...
    <div className="flex px-2 py-2" style={{ minHeight: h }}>
      ...
      <div className="flex-1 relative" style={{ minWidth: 80 }}>
        <svg width="100%" height="100%" viewBox={`0 0 200 ${h}`} preserveAspectRatio="none">
          ...
          <text x="100" y={h / 2} textAnchor="middle" fill="#FE500025" fontFamily="Geist Mono, monospace" fontSize="6">BUS</text>
          {left.map((ch, i) => {
            const kt = KNOWLEDGE_TYPES[ch.knowledgeType] ?? KNOWLEDGE_TYPES['evidence'];
            const y = 14 + i * 28;
