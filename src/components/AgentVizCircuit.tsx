  ...
  return (
    <div className="w-full" style={{ background: t.surfaceOpaque, borderTop: '2px solid #FE5000' }}>
      ...
      <div className="flex flex-col items-center py-3 px-4">
        <span style={{ fontSize: 26 }}>{agentMeta.icon || '🤖'}</span>
        <span style={{ fontFamily: "'Geist Mono', monospace", fontSize: 14, fontWeight: 700, color: t.textPrimary, letterSpacing: '0.1em', textTransform: 'uppercase' }}>
          {agentMeta.name || 'Untitled Agent'}
        </span>
        <span style={{ fontSize: 13, color: '#FE5000', fontFamily: "'Geist Mono', monospace", marginTop: 2 }}>
        ...
        {instructionState.objectives.primary && (
          <span style={{ fontSize: 12, color: t.textMuted, marginTop: 4, textAlign: 'center', maxWidth: 400 }}>
            {instructionState.objectives.primary}
          </span>
        )}
      </div>
      ...
      <div className="flex gap-2 justify-center pb-2 flex-wrap px-4">
        {constraintList.map((c) => (
          <span
            key={c.label}
            style={{
              fontSize: 12, fontFamily: "'Geist Mono', monospace", padding: '2px 8px', borderRadius: 4, letterSpacing: '0.04em',
      ...
      <div className="flex" style={{ minHeight: boardH }}>
        ...
        <div className="flex flex-col gap-1 py-2 px-3 shrink-0" style={{ width: 200 }}>
          <div style={{ fontSize: 8, color: t.textDim, fontFamily: "'Geist Mono', monospace", letterSpacing: '0.08em', textTransform: 'uppercase', paddingLeft: 8, paddingBottom: 2 }}>📥 KNOWLEDGE</div>
          {activeChannels.length === 0 && <span style={{ fontSize: 13, color: t.textFaint, paddingLeft: 8 }}>No sources</span>}
            ...
            return (
              <div key={ch.sourceId} className="flex items-center gap-2 px-2 py-1 rounded" style={{ background: '#ffffff04', border: '1px solid #ffffff06' }}>
                ...
                <div className="flex-1 min-w-0 flex flex-col gap-0.5">
                  <span style={{ fontSize: 13, color: t.textSecondary, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{ch.name}</span>
                ...
                <span style={{ fontSize: 8, fontFamily: "'Geist Mono', monospace", padding: '1px 4px', borderRadius: 3, background: `${kt.color}15`, color: kt.color, textTransform: 'uppercase', fontWeight: 600, flexShrink: 0 }}>
        ...
        <div className="flex-1 relative" style={{ minWidth: 120 }}>
          <svg width="100%" height="100%" viewBox={`0 0 300 ${boardH}`} preserveAspectRatio="none">
            ...
            <text x="150" y={boardH / 2} textAnchor="middle" fill="#FE500030" fontFamily="Geist Mono, monospace" fontSize="7" letterSpacing="0.1em">CONTEXT BUS</text>
        ...
        <div className="flex flex-col gap-1 py-2 px-3 shrink-0" style={{ width: 180 }}>
          <div style={{ fontSize: 8, color: t.textDim, fontFamily: "'Geist Mono', monospace", letterSpacing: '0.08em', textTransform: 'uppercase', textAlign: 'right', paddingRight: 8, paddingBottom: 2 }}>🔧 TOOLS & SKILLS</div>
          {rightPins.length === 0 && <span style={{ fontSize: 13, color: t.textFaint, textAlign: 'right', paddingRight: 8 }}>None configured</span>}
          {rightPins.map((pin) => {
            ...
            return (
              <div key={pin.name} className="flex items-center gap-2 px-2 py-1 rounded justify-end" style={{ background: '#ffffff04', border: '1px solid #ffffff06' }}>
                {pin.badge && (
                  <span style={{ fontSize: 8, fontFamily: "'Geist Mono', monospace", padding: '1px 4px', borderRadius: 3, background: `${color}15`, color, textTransform: 'uppercase', fontWeight: 600 }}>
                    {pin.badge}
                  </span>
                )}
                <span style={{ fontSize: 13, color: t.textSecondary }}>{pin.name}</span>
      ...
      <div className="flex items-center justify-center gap-0 py-3 px-6" style={{ borderTop: `1px solid ${t.borderSubtle}` }}>
        {workflowSteps.length === 0 && <span style={{ fontSize: 13, color: t.textFaint }}>No workflow defined</span>}
            ...
            <div key={step.id} className="flex items-center">
              <div className="flex flex-col items-center gap-1">
                <div style={{
                  ...
                  fontFamily: "'Geist Mono', monospace", fontSize: 12, fontWeight: 700, color: isLoop ? '#f1c40f' : '#FE5000',
                }}>
                  {i + 1}
                </div>
                <span style={{ fontSize: 8, color: t.textDim, fontFamily: "'Geist Mono', monospace", maxWidth: 60, textAlign: 'center', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                  {step.label || step.action || 'Step'}
                </span>
                {isLoop && <span style={{ fontSize: 8, color: '#f1c40f80', fontFamily: "'Geist Mono', monospace" }}>↻ max {step.loopMax || 3}</span>}
              </div>
      ...
      <div className="flex items-center gap-3 px-6 pb-3">
        <span style={{ fontSize: 12, color: t.textDim, fontFamily: "'Geist Mono', monospace", letterSpacing: '0.1em', textTransform: 'uppercase', flexShrink: 0 }}>CONTEXT</span>
        ...
        <span style={{ fontSize: 12, color: '#FE5000', fontFamily: "'Geist Mono', monospace", fontWeight: 700, flexShrink: 0 }}>
