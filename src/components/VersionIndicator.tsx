  ...
  return (
    ...
    >
      <button
        ...
        className="w-full flex items-center gap-2 px-3 py-2 cursor-pointer border-none bg-transparent text-left"
      >
        <ChevronRight
          size={10}
          style={{ color: t.textDim, transform: expanded ? 'rotate(90deg)' : 'none', transition: 'transform 0.15s' }}
        />
        <span className="text-[12px] font-bold" style={{ fontFamily: "'Space Mono', monospace", color: isCurrent ? '#FE5000' : t.textPrimary }}>
          v{v.version}
        </span>
        {isCurrent && <Badge variant="success" size="sm" dot>CURRENT</Badge>}
        <span className="flex-1 text-[10px] truncate" style={{ color: t.textMuted }}>{v.label}</span>
        ...
        <div className="flex flex-col gap-1.5 p-3 max-h-[60vh] overflow-y-auto">
          {versions.length === 0 && (
            <div className="py-8 text-center">
              ...
              <div className="text-[10px]" style={{ color: t.textFaint }}>No versions yet</div>
